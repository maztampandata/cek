/**
 * tes.js
 * - UI: persis sama dengan "worker 4 serba otomatis" (tidak diubah)
 * - Backend: ambil SG + ID, rotate 1 random ip:port, hasilkan config TLS & NTLS (trojan/vmess/ss)
 * - Endpoints:
 *    /           -> UI (worker 4 HTML unchanged)
 *    /sub        -> JSON hasil rotate 1 config (use ?domain=example.com optional)
 *    /SG.txt     -> raw parsed SG list JSON
 *    /ID.txt     -> raw parsed ID list JSON
 *    websocket support and reverse proxy preserved (from your worker core)
 *
 * NOTE: pastikan runtime Worker-mu support cloudflare:sockets jika mau gunakan websocket/tcp features.
 */
...
const horse = "dHJvamFu"; // trojan (base64)
const flash = "dmxlc3M="; // vmess (base64)
const v2 = "djJyYXk="; // v2ray (base64)
const neko = "Y2xhc2g="; // clash (base64)
    ...
    <script>
        ...
        async function loadConfig() {
            try {
                ...
                // call /sub which returns a single rotated config JSON
                const domain = location.hostname || '';
                const response = await fetch('/sub?domain=' + encodeURIComponent(domain));
                ...
                if (data && !data.error) {
                    // show UUID (we'll extract UUID from one of configs if possible)
                    let uuid = 'n/a';
                    try {
                        const vmess = data.config_tls && data.config_tls.vmess;
                        if (vmess) {
                            // vmess:// may contain username@domain:port - we used username=uuid in generation
                            const m = vmess.match(/\/\/([^@]+)@/);
                            if (m) uuid = m[1];
                        }
                    } catch(e) { uuid = 'n/a'; }

                    uuidValue.innerHTML = uuid;
                    proxyValue.innerHTML = data.ip + ':' + data.port + ' (' + data.org + ')';
                    configBox.textContent = JSON.stringify(data, null, 2);
                    currentConfig = JSON.stringify(data, null, 2);
                    activeProxy.innerHTML = data.ip + ':' + data.port;
                    
                    // update proxy list UI by fetching SG and ID lists
                    ...
                    proxyCount.innerHTML = pool.length + ' proxies available';
                    proxyList.innerHTML = '';
                    pool.forEach(p => {
        ...
        // Generate VLESS config (calls /sub too but could call /sub?format=vless later)
        async function generateVlessConfig() {
            try {
                ...
                if (data && !data.error) {
                    // show a vless-like (we use vmess/trojan/ss from result)
                    const vlessApprox = data.config_tls && (data.config_tls.trojan || data.config_tls.vmess) || '';
                    configBox.textContent = vlessApprox;
                    currentConfig = vlessApprox;
                    showNotification('VLESS (approx) config generated successfully!');
                } else {
        ...
        async function refreshProxies() {
            try {
                ...
                // Force reload config to generate proxy new
                await loadConfig();
